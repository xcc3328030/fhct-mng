<template>
  <div class="remoteRecipes">
    <div class="common-search-input">
      <el-input placeholder="搜索患者" v-model="input_remote" class="input-with-icon" @keyup.enter.native="searchContactOrDoctor()">
    ...
    <div class="common-search-order">
      ...
      <remote-table :data="orders" v-loading="loading" v-show="!defaultImg"></remote-table>
      <el-pagination
        v-show="!defaultImg"
        class="pagination"
        background
...
export default {
  ...
  methods: {
    ...
    handleSearch (param) {
      ...
      PARAM['hospitalId_must_match'] = this.$tools.getSessionStorage('user_details_data').hospitalId;
      this.$services.fetchRemoteList(PARAM)
      .then((res) => {
        this.loading = false;
    ...
    searchContactOrDoctor () {
      ...
      let param = {
        'contactDO.name_should_match': this.input_remote || undefined,
        'doctorName_should_match': this.input_remote || undefined,
    ...
    searchInProgress () {
      let param = {
        orderStatus_should_match: ['WAITING'],
        page: this.page,
        size: this.size,
      }
      this.handleSearch(param);
    ...
    searchCompleted () {
      let param = {
        orderStatus_should_match: ['SUCCEED_OPENED','REFUSED'],
        page: this.page,
        size: this.size,
      }
      this.handleSearch(param);
